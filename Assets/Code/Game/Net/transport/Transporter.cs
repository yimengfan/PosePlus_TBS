...
namespace Pomelo.DotNetClient
{
    ...
    public class Transporter
    {
        ...
        {
            //UnityEngine. JDeBug.I.Log("sendCallback " + this.transportState);
        ...
        private void endReceive(IAsyncResult asyncReceive)
        {
            ...
            try
            {
                //JDeBug.I.Log("begin endreceive");
                SocketError errorCode = SocketError.Success;
                int length = socket.EndReceive(asyncReceive,out errorCode);
                //JDeBug.I.Log("end receive ,length:" + length);
                if(errorCode != SocketError.Success)
                {
                    UnityEngine.Debug.LogError("error code:" + errorCode);
                }
                this.onReceiving = false;

                if (length > 0)
                {
                    // JDeBug.I.Log("begin process byte");
                    processBytes(state.buffer, 0, length);
                    // JDeBug.I.Log("trans port:" + this.transportState);
                    //Receive next message
                    if (this.transportState != TransportState.closed)
                    {
                      //  JDeBug.I.Log("do next receive");
                        receive();
                    }
                }
                else
